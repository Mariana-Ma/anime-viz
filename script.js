...
Promise.all([
    d3.csv("anime.csv", d3.autoType),
    d3.csv("anime_primary_genre.csv", d3.autoType)
]).then(([anime, genres]) => {
...
function buildChart(anime, genres) {
    ...
    const data = anime
        ...
        .slice(0, 300) // top 200 popular anime
        .map(d => ({
            title: d.title,
            score: d.score,
...
function drawScatter(data) {
    ...
    // --- Attach click listeners to SVG circles ---
    ...
    dots.forEach((dot, i) => {
        ...
        dot.addEventListener("mouseover", () => {
            dot.setAttribute("r", 18);          // grow the dot
        });

        dot.addEventListener("mouseout", () => {
            // Reset size depending if it's selected or not
            if (dot === selectedDot) {
                dot.setAttribute("r", 18);        // selected size
            } else {
                dot.setAttribute("r", 12);        // normal size
            }
        });
